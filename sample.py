    ...
    if grid[row][col]==0:
      for value in range (1,10):
        ...
        if not(value in grid[row]):
          ...
          if not value in ([grid[i][col] for i in range(9)]):
            #Identify which of the 9 squares we are working on
            square=[]
            if row<9:
              if col<9:
                square=[grid[i][int(col/3)*3:int(col)*3+1] for i in range(int(row/3),int(row/3)*3+1)]
              else:
                square=[grid[i][6:9] for i in range(int(row/3),int(row/3)*3+1)]
            else:
              if col<9:
                square=[grid[i][int(col/3)*3:int(col)*3+1] for i in range(6,9)]
              else:
